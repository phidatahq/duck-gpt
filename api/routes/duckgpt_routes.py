...
duckgpt_router = APIRouter(prefix=endpoints.DUCKGPT, tags=["duckgpt"])
...
@duckgpt_router.post("/query", response_model=DuckGptResponse)
def duckgpt_query(duckgpt_request: DuckGptRequest):
    ...
    # Get the output
    if "output" in result:
        return DuckGptResponse(messages=duckgpt_messages, output=result["output"])
    else:
        return DuckGptResponse(messages=duckgpt_messages, output="No output")
